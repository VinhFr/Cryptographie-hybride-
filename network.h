...
/* =================================================================== */
/* Ici, j'utilise send_n_data (ou recv_n_data) au lieu de la fonction  */
/* send (recev) simple pour garantir que tous les octets demandés sont */
/* effectivement envoyés (ou recu) sur le socket.                      */
/*                                                                     */
/* BN = Big number (Grand nombre)                                      */
/*                                                                     */
/* La fonction send (ou recev) peut ne pas envoyer (ou recevoir)       */
/* toutes les données en une seule fois, notamment lorsque             */
/* la taille des données est grande ou à cause des limites             */
/* du buffer réseau.                                                   */
/* Par conséquent, il est nécessaire de boucler et d’envoyer           */
/* (ou recevoir) les données restantes                                 */
/* jusqu’à ce que la totalité soit transmise.                          */
/*                                                                     */
/* send_n_data (ou recv_n_data) gère ce comportement en répétant       */
/* les appels à send, ajustant le pointeur de données et               */
/* la longueur restante à chaque itération,                            */
